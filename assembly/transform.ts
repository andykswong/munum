import { Float, Mat4, Int, ReadonlyMat4, ReadonlyVec3, Vec3 } from '@andykswong/munum-types';
...
export function rotateX(theta: Float, out: Mat4 = mat4.create()): Mat4 {
  mat4.id(out);
  out[5] = out[10] = Math.cos(theta) as Float;
  out[6] = out[9] = Math.sin(theta) as Float;
  out[9] *= -1;
  return out;
}
...
export function rotateY(theta: Float, out: Mat4 = mat4.create()): Mat4 {
  mat4.id(out);
  out[0] = out[10] = Math.cos(theta) as Float;
  out[2] = out[8] = Math.sin(theta) as Float;
  out[2] *= -1;
  return out;
}
...
export function rotateZ(theta: Float, out: Mat4 = mat4.create()): Mat4 {
  mat4.id(out);
  out[0] = out[5] = Math.cos(theta) as Float;
  out[1] = out[4] = Math.sin(theta) as Float;
  out[4] *= -1;
  return out;
}
...
export function rotationOf(m: ReadonlyMat4, out: Quat = quat.create()): Quat {
  ...
  out[3] = (Math.sqrt(Math.max(0, 1 + m00 + m11 + m22)) as Float) / 2;
  out[0] = copysign((Math.sqrt(Math.max(0, 1 + m00 - m11 - m22)) as Float) / 2, m[6] / v1[1] - m[9] / v1[2]);
  out[1] = copysign((Math.sqrt(Math.max(0, 1 - m00 + m11 - m22)) as Float) / 2, m[8] / v1[2] - m[2] / v1[0]);
  out[2] = copysign((Math.sqrt(Math.max(0, 1 - m00 - m11 + m22)) as Float) / 2, m[1] / v1[0] - m[4] / v1[1]);
...
export function lookAt(
  ...
  out[9] = u[2];
  out[10] = -v[2];
  out[12] = -vec3.dot(n, eye);
  out[13] = -vec3.dot(u, eye);
  out[14] = vec3.dot(v, eye);
  out[3] = out[7] = out[11] = 0;
  out[15] = 1;
  return out;
}
...
export function direction(yaw: Float, pitch: Float, out: Vec3 = vec3.create()): Vec3 {
  ...
  out[0] = cosPitch * Math.cos(yaw) as Float;
  out[1] = Math.sin(pitch) as Float;
  out[2] = cosPitch * Math.sin(yaw) as Float;
  return out;
}
